...
<ol class="breadcrumb">
  ...
  <li class="active">Modifier Les Résultats </li>
</ol>
...
<ol class="breadcrumb">
    ...
    <select class="form-control chosen-select-deselect " data-placeholder="Filière" data-toggle="tooltip" data-placement="top" title="Recherche par Filière" id="filiere"  >
       ...
       <td style="width:10px"></td>
        <td style="width:100px">
             <select class="form-control  chosen-select-no" data-placeholder="Mention"  id="mention" class="label label-info"   data-toggle="tooltip" data-placement="top" title="Recherche par Mention" >  
               <option value=""></option>
        ...
        <td style="width:10px"></td>
        <td style="width:100px">
             <select class="form-control  chosen-select-no" data-placeholder="Validité"  id="validite" class="label label-info"   data-toggle="tooltip" data-placement="top" title="Recherche par Validité" >  
               <option value=""></option>
...
<div class="col-sm-offset-0 col-sm-12">
      ...
      <legend>  &nbsp;<i class="fa fa-pencil-square-o fa-lg"></i>&nbsp;<i>Modifier Les Résultats</i></legend>
...
<br>
    ...
    <div>
            ...
            <form class="form-horizontal" role="form" method="POST" action="{{ url('stagiaire/1')}}">
 ...
 <table id="resultat" class="table table-bordered table-striped table-hover table-condensed" cellspacing="0" width="100%">
    ...
    <tbody>
                 ...
                 <tr>
                    ...
                    <td style="width:20px">{{$user->etId}}</td>
                    <td style="width:50px">{{$user->etudiant_cin}}</td>
                    <td style="width:50px">{{$user->etudiant_nom}}</td>
                    <td style="width:50px">{{$user->etudiant_prenom}}</td>  
                    ...
                    <td style="width:100px">
                     ...
                     <td style="width:100px">{{$user->etudiant_niveau}}</td> 
                     <td style="width:100px">{{$user->stage_title}}</td>
                     <td style="width:100px">{{$user->type_nom}}</td>
                     <td style="width:100px">{{$user->valide}}</td>
                     <td style="width:100px">{{$user->grade}}</td>

                  
                    <td style="width:50px">
                         
                      <input type="number" class="form-control" name="not{{$user->id}}"   value="{{$user->moyenne}}"   min="0" max="20"  step="any"> 
                 
                    </td style="width:100px"> 
                     <td><textarea class="form-control" rows="3"  LANG="fr" name="appreciation{{$user->id}}" >{{$user->appreciation }}</textarea>
                    </td>
...
$(document).ready(function() {
 ...
 $('select#mention').change( function() { table.column(9).search($(this).val()).draw(); } );
 $('select#validite').change( function() { table.column(8).search($(this).val()).draw(); } );
